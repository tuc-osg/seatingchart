...
\begin{commands}
  \command{scConfig}This command can be used to set all keys mentioned in
...
\begin{example}[compile,program=lualatex,pages=1,add-frame,code-sep=\bigskip,graphics={scale=.25}]
  ...
  \usepackage[shape=rectangle, rows = 8, seats per row=9]{seatingchart}
  \begin{document}
    \scSetAisle{5}
    \scRemoveSeatAt{1}{1}
    \scDrawSeating
...
The \code{0} in the major version number indicates this.
...
The package is designed for creating separate seating plans and therefore cannot
...
There are plans to include additional rooms from TU Chemnitz in future patches.
Your collaboration by sending information about seating layouts or
corresponding patches of \code{seatingchart-*.tsr} is expressly welcome.

There are currently no known bugs, but some will certainly exist.
