...
\providecommand{\packageversion}{v0.2}
...
\begin{options}
  ...
  \keyval-{rows}{Anzahl der Sitzreihen}\Default*!{}\vspace{-.7\baselineskip}
...
\begin{options}
  ...
  \keyval-{seat neighbor distance}{Abstand}\Default*{2pt}\vspace{-.7\baselineskip}
  ...
  nicht \code{none} ist.
  \keyval-{empty seat background color}{Farbe}\Default*{lightgray!20}\vspace{-.7\baselineskip}
  \keyval-{empty seat border color}{Farbe}\Default*{lightgray}\vspace{-.7\baselineskip}
  \keyval-{assigned seat background color}{Farbe}\Default*{lightgray!30}\vspace{-.7\baselineskip}
  ...
  Schlüssel
  \keyval-{seat background color}{Farbe}\Default*\vspace{-.7\baselineskip}
  \keyval-{seat border color}{Farbe}\Default
  verwendet werden.
  \keyval{assigned seat label font}{Fontbefehl}\Default*{\cs*{small}}\vspace{-.7\baselineskip}
...
\begin{options}
  ...
  \keyval-{start row}{Nummer}\Default*{1}\vspace{-.7\baselineskip}
  ...
  \keybool{ignore aisle}\Default*{false}\vspace{-.7\baselineskip}
...
unterstützen.
\begin{commands}
    \command{tucsSeatingList}[\oarg{Eingabedatei}\marg{Ausgabedatei}]\vspace{-.7\baselineskip}
    ...
    Plätze, jeweils einen pro Zeile.
    Wenn die Ausgabe einer solchen Datei gewünscht ist, empfiehlt es sich nicht
    (wie bei der Vorgabeeinstellug) in
    der Formatzeichenkette von \opt{assigned seat label} Befehle zu
    verwenden, die \LaTeX-Skips oder -Glues oder andere \LaTeX-Spezifika
    erzeugen.
    Solche werden in der Dateiausgabe ignoriert oder in Leerzeichen gewandelt.

    In der Sternvariante werden zusätzlich in jeder Zeile kommasepariert noch
    die absoluten Koordinaten (Reihe, Platznummger) ausgegeben, bezogen auf das
    Grundlayout.

    Wenn eine Eingabedatei vorhanden ist, wird ihr Inhalt zweilenweise den
    erzeugten Zeilen zugeordnet. Diese Eingabedatei könnte beispielsweise die Namen
    oder die Immatrikulationsnummern von Studierenden enthalten, die dann in der
